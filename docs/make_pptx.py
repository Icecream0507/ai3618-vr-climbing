...
s = slide(); header_band(s)
kicker(s, "工程验证 · 不是「写完就交」")
txt(s, Inches(0.55), Inches(0.85), Inches(12.4), Inches(0.7),
    [[("脚本机器人在引擎里", 28, INK, True), ("端到端", 28, RED, True), ("跑通全闭环", 28, INK, True)]])
...
txt(s, Inches(0.9), Inches(4.35), Inches(11.5), Inches(0.95),
    [[("机器人抓到的真 bug（加分点）", 15, WHITE, True)],
     [("CharacterController.minMoveDistance 默认 0.001m 会吞掉慢速攀爬的每帧位移，导致「爬不动」——纯看代码发现不了，只有真在引擎里跑才暴露。已修复并纳入回归。", 13, WHITE, False)]],
    space_after=4, line=1.3)
speak(s, "机器人像真玩家一样爬墙，断言 10 条核心行为；它跑的时候抓到一个只有实跑才暴露的真 bug。说明我们不是写完就交。（P4 补 XR 接线）")
footer(s, "工程验证")
print("slide 10 done")
...
kicker(s, "评测计划与未来工作")
txt(s, Inches(0.55), Inches(0.85), Inches(12.4), Inches(0.7),
    [[("系统已实现，", 28, INK, True), ("数据诚实留空", 28, RED, True), ("——交的是可证伪的设计", 28, INK, True)]])
bullets(s, Inches(0.6), Inches(1.95), Inches(6.4), Inches(2.0), [
  [("被试内对比", True, REDD), ("（同线路、顺序平衡）：A 纯手 vs B 平衡+脚法。", False, INK)],
  [("指标：", True, REDD), ("登顶时间 · 跌落按原因分（松手/失衡/体力）· 平衡余量曲线 · NASA-TLX · SSQ · 真实感量表（n≈5–8）。", False, INK)],
], size=14, gap=11)
c = card(s, Inches(0.6), Inches(3.85), Inches(6.4), Inches(1.5), red_bg=True)
txt(s, Inches(0.85), Inches(4.0), Inches(5.9), Inches(1.2),
    [[("核心假设 H", 14, WHITE, True)],
     [("B 的 真实感↑ · 挑战↑，但 晕动 SSQ 不升——挑战来自决策难度而非运动冲突，这是市面 VR 攀岩都没有的差异化卖点。", 13, WHITE, False)]],
    space_after=4, line=1.3)
txt(s, Inches(7.3), Inches(1.95), Inches(5.4), Inches(0.4), [[("未来工作", 14, RED, True)]])
bullets(s, Inches(7.3), Inches(2.45), Inches(5.4), Inches(3.0), [
  [("① 真的把评测跑了，填上空表", False, INK)],
  [("② 一维区间 → 2D 支撑多边形（加前后向平衡）", False, INK)],
  [("③ 玩家自主选脚点（取代纯自动吸附）", False, INK)],
  [("④ 用手柄位置改进重心估计", False, INK)],
  [("⑤ 真机 Quest 构建 + 帧率 / 舒适度 vignette", False, INK)],
  [("提交：GitHub 链接 + demo 视频（7/2）", False, MUT)],
], size=13, gap=8)
speak(s, "我们没假装有结果。机制正确性已用仿真闭环验证；人因数据是诚实留空的占位——这正是这份报告的诚信所在。")
footer(s, "评测 · 未来工作")
print("slide 12 done")
