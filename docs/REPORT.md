...
Every hold carries two pieces of metadata. A **role** (`Hand`, `Foot`, or `Either`) and **type** (`Normal`, `Finish`, `Fragile`, `Rest`) are stored for route authoring and a colour legend (yellow ≈ hand, orange ≈ foot, purple = either, green = finish, red = fragile, blue = rest). **Roles are visual hints only** — like real rock, any hold can accept a hand grip or a virtual foot; the old hand-only / foot-only gates were removed so players are not blocked by colour. Types still drive behaviour: `Fragile` breaks after a short hold time, `Rest` suppresses stamina drain, `Finish` completes the route.
...
Summit VR is built in **Unity 2022.3 LTS** (verified on Tuanjie 2022.3.62t7) with the **Universal Render Pipeline (URP 14.1.0)**, chosen because URP is the recommended mobile-class pipeline for standalone Quest hardware (single-pass instanced rendering, low draw-call budget). The repo declares the modern XR package stack for future hardware builds: **OpenXR 1.14.3**, **XR Interaction Toolkit (XRI) 3.2.1**, **XR Management 4.4.0**, **Input System 1.14.3**, and optional **XR Hands 1.4.0**; TextMeshPro 3.0.9 / uGUI drive the world-space HUD. The *design* target is **Meta Quest (head + two controllers only, no foot tracking)**, which is the constraint the entire balance-and-footwork design is written around.

For this course submission the **critical path is simulation-only** (no physical headset required): a scripted robot and a mouse/keyboard driver exercise the same gameplay stack as a future XR rig would. OpenXR/XRI packages and loader assets are committed so a teammate can wire an XR Origin later (`docs/SETUP.md` §1–2), but that path is optional per the course brief.

A deliberate decision in the code is that climbing input is read through an `InputActionProperty` (the grip / *Select Value* action) rather than through a specific XRI interactor class or Unity's XRI Climb Provider. This decouples our logic from the exact XRI version: a controller's grip value is read each frame and thresholded, so the same scripts run unchanged across XRI point releases, in the headless robot, and in keyboard/mouse play scenes. Simulation drivers (`SimulatedClimber`, `PlayInputController`) bypass actions entirely via `ClimbingHand.overrideGrip` while still moving `handTransform` — the same contract real controllers satisfy. The only XR APIs used directly today are the legacy `UnityEngine.XR.InputDevices` haptics helper (for an optional rumble pulse on grab) and `XRNode` tagging on each hand; no gameplay script imports `UnityEngine.XR.Interaction.Toolkit`.
...
**`ClimbingHand`** (one per controller) is the input and reach layer. Each frame it reads the grip action's float value and compares it to `gripThreshold` (0.6). On a rising edge it runs a non-allocating `Physics.OverlapSphereNonAlloc` of radius `grabRadius` (0.12 m) on the *Hold* layer, picks the nearest valid `ClimbHold` (skipping broken holds; **role is not filtered** — any colour can be grabbed), latches it as `CurrentHold`, increments the hold's gripper count, fires `Grabbed`, and pulses haptics. On the falling edge — or if the held hold breaks or is disabled mid-grip — it releases. It exposes `IsGripping`, `CurrentHold`, and `HandPosition` (the tracked controller transform), which is all the controller needs. A static eight-slot `Collider[]` buffer is reused for the overlap query so grabbing allocates no garbage.
...
Two "virtual feet" are solved each frame. A stance base is estimated `bodyDrop` (1.15 m) below the HMD, offset left/right by `stanceHalfWidth` (0.22 m) along the lateral axis. Each foot runs an overlap query of radius `footReach` (0.45 m) on the Hold layer for the nearest hold (role is not filtered). The key behaviour is **foot-gluing**: a planted foot keeps its current hold as long as that hold stays within `footReach` of the rest position, and only re-snaps to a lower hold once the body climbs past it. Planted feet optionally drive simple visual markers, and — critically — their `GripPoint`s are appended to the balance contacts via `CollectContacts`, so placing a foot literally widens the lateral support span and re-centres the climber. `PlantedCount` is what tells `ClimbController` that a feet-only hang is valid, and `DropAll` clears both feet on a peel-off or respawn.

### 4.6 Holds: `ClimbHold` and roles/types

`ClimbHold` tags a GameObject as climbable and carries gameplay metadata. **Roles** {`Hand`, `Foot`, `Either`} tint the hold and hint intended use; **types** {`Normal`, `Finish`, `Fragile`, `Rest`} drive behaviour: `Fragile` holds track cumulative held-time via a gripper count and `Break` (deactivate) after `breakAfterSeconds` (1.5 s); `Rest` holds suppress stamina drain; `Finish` marks the topping-out hold. The colour legend is rendered in editor gizmos and by the route builder's material assignment. `GripPoint` (an optional anchor transform, else the object's position) is the single world point every other system anchors and measures against.
...
### 4.10 Validation without a headset (simulation path)

Because climbing input is abstracted behind `ClimbingHand` (`handTransform` + grip value or `overrideGrip`), the **full gameplay stack is validated without any XR hardware**. Three complementary paths are implemented:

1. **Headless end-to-end check** (`VRClimb ▸ Run Headless Check`, or `-executeMethod VRClimb.EditorTools.HeadlessCheck.Run`). This runs the `ClimbMath` self-test (9 assertions), builds `SimTest.unity` with a plain camera rig wired through `PlayerClimberSetup`, and drives a **`SimulatedClimber`** robot through peel-off → fall → respawn → route-0 summit. Exit code 0 and `Logs/headless-check.txt` report **9/9 + 10/10 PASS** on the current baseline (~6 s to summit, one scripted slip).
2. **Human play without a headset** — editor menus build **`Play.unity`** (third-person) and **`VR.unity`** (first-person; mouse/keyboard, not OpenXR) with the same **`PlayInputController`** driver: left/right click grab, A/D lean, W/S pull. This reuses the identical `overrideGrip` contract as the robot, so "robot passes" implies "human can play the same stack."
3. **Unattended demo capture** — `DemoBuild.Record` batch-renders `Demo.unity` to `Logs/frames/*.jpg` for ffmpeg assembly into `Demo/SummitVR_demo.mp4` (see `docs/TESTING.md` §4).

**Future work (optional, not on the course critical path):** import XRI Starter Assets, drop the **XR Device Simulator** prefab, bind each `ClimbingHand.gripAction` to XRI *Select Value*, and enable OpenXR on PC/Android for Quest Link or standalone builds (`docs/SETUP.md` §1–2, §6). The committed OpenXR loader assets are placeholders until that wiring is done.
